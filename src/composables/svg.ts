...
export type getSvgFn = (settings: Settings, appearance?: SvgAppearance, width?: number, height?: number, background?: boolean) => string

export const getBlurSvg: getSvgFn = (settings: Settings, appearance?: SvgAppearance, width?: number, height?: number, background?: boolean): string => {
  ...
  const animation = [
    ...
    `${ANIMATION_TIME / (settings.during * settings.texts.length) * 100}%{opacity:1;filter:blur(0px);transform:translateY(0px);animation-timing-function:linear}`,
    `${1 / settings.texts.length * 100}%{opacity:1;filter:blur(0px);transform:translateY(0px);animation-timing-function:ease}`,
    `${(settings.during + ANIMATION_TIME) / (settings.during * settings.texts.length) * 100}%{opacity:0;filter:blur(5px);transform:translateY(-20px);animation-timing-function:linear}`,
...
export const getFadeSvg: getSvgFn = (settings: Settings, appearance?: SvgAppearance, width?: number, height?: number, background?: boolean): string => {
  const _appearance = appearance === undefined ? settings.export.svg.appearance : appearance
  const _width = width === undefined ? settings.export.size.width : width
  const _height = height === undefined ? settings.export.size.height : height
  const _background = background === undefined ? settings.export.svg.background : background
  ...
  const animation = [
    `@keyframes a{0%{opacity:0;animation-timing-function:ease}`,
    `${ANIMATION_TIME / (settings.during * settings.texts.length) * 100}%{opacity:1;animation-timing-function:linear}`,
    `${1 / settings.texts.length * 100}%{opacity:1;animation-timing-function:ease}`,
    `${(settings.during + ANIMATION_TIME) / (settings.during * settings.texts.length) * 100}%{opacity:0;animation-timing-function:linear}`,
    `100%{opacity:0;animation-timing-function:ease}}`,
  ].join('')
